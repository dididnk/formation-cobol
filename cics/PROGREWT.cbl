       ...
       PROGRAM-ID. PROGREWT.
      *****************************************************************                                              
      * REWRITE : PROGRAMME QUI MET À JOUR UN ENREGISTREMENT (ID: 001)*
      *****************************************************************
       DATA DIVISION.                                                   
      ...
      * VÉRIFIE SI L'ID DU CLIENT N'EXISTE PAS    
           ...
           END-IF.    
      * QUELQUES INFOS À MODIFIER (NOM ET PRENOM)                               
           MOVE "RONALDINHO"  TO WS-NOMCPT.                             
           MOVE "LEO MESSI "  TO WS-PRNCPT.  
      * MET À JUR LE FICHIER
           EXEC CICS REWRITE FILE('FCLIENT')
               FROM(WS-REC-DATA) RESP(WS-RESCODE)
           END-EXEC.  
      * AFFICHER LE RESULTAT
           INITIALIZE WS-MSG.
           IF WS-RESCODE = DFHRESP(NORMAL)
               MOVE "CLIENT 001 MODIFIÉ AVEC SUCCES" TO WS-MSG
           ELSE
